...
namespace RandomEnemiesSize.Patches
{
    ...
    internal class PatchEnemySize
    {
        ...
        {
            Debug.Log(
                $"ENEMY ({__instance.gameObject.name}) SPAWNED WITH RANDOM SIZE {__instance.transform.localScale.ToString()} AND HP {__instance.enemyHP}");
        }

        private static void PatchStartOld(EnemyAI __instance)
        {
